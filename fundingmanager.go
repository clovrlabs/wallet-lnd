...
func (f *fundingManager) advancePendingChannelState(
	...
	fakeID, err := f.fakeIDSManager.generateFakeID(pendingChanID)
	if err != nil {
		return err
	}
...
func (f *fundingManager) waitForFundingConfirmation(
	...
	// We make sure we wait at least one confirmation.
	if numConfs == 0 {
		numConfs = 1
	}
...
func (f *fundingManager) handleFundingConfirmation(
	...
	// used.
	if !isFakeShortChannelID(completeChan.ShortChannelID) {
...
func newFakeIDsManager(channels []*channeldb.OpenChannel,
	...
	for _, c := range channels {
		...
		if isFakeShortChannelID(shortID) {
			idsToChannels[shortID.ToUint64()] = c
		}
	}

	// We prune each fake id in our bucket that is not associated with
	// a pending or opened channel.
	err := kvdb.Update(db, func(tx kvdb.RwTx) error {
		...
		return fakeIDsBucket.ForEach(func(k, v []byte) error {
			...
			if !exists {
				...
				err := graph.DeleteChannelEdges(shortID)
				if err != nil {
					fndgLog.Debugf("failed to delete fake edge, probably "+
						"already deleted %v", err)
				}
				fakeIDsBucket.Delete(k)
			}
			return nil
		})
	})
...
func (f *fakeIDsManager) generateFakeID(
	pendingID [32]byte) (lnwire.ShortChannelID, error) {

	// We generate the short channel id from the random pending id
	// We make sure the blockHeight will be under 500,000 shifting
	// 8 bits.
	shortChanID := byteOrder.Uint64(pendingID[:8])
	shortChanID = shortChanID >> 8
	fakeID := lnwire.NewShortChanIDFromInt(shortChanID)
...
func (f *fakeIDsManager) releaseFakeID(
	...
	err := kvdb.Update(f.db, func(tx kvdb.RwTx) error {
		...
		var id = make([]byte, 8)
		byteOrder.PutUint64(id[:8], fakeID.ToUint64())
		fakeIDsBucket.Delete(id)
		return nil
	})
	return err
}

func isFakeShortChannelID(id lnwire.ShortChannelID) bool {
	return id.BlockHeight < 500000
}

...
func (s *Server) HtlcInterceptor(stream Router_HtlcInterceptorServer) error {
	...
	fromSwitchInterceptor := func(packet htlcswitch.InterceptedForward) bool {
		log.Infof("holding intercepted packet %v", packet)
	...
	// Register our interceptor and make sure we resolve the held packets
	// on disconnect.
...
// onInterceptorDisconnected removes all previousely held forwards from
...
// drain removes all previousely held forwards and allows to pass a callback

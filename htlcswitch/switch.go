...
func (s *Switch) UpdateShortChanID(chanID lnwire.ChannelID) error {
	...
	if oldID.IsFake() && oldID != shortChanID {
		if err := s.replaceChanOldCiruits(link, oldID, shortChanID); err != nil {
			return err
		}
	}
...
func (s *Switch) replaceChanOldCiruits(
	...
	for _, h := range pendingHtlcs {
		...
		// fetching all opened circuits for this payment hash
		openCiruits := s.circuits.LookupByPaymentHash(h.RHash)
		log.Infof("lookup by payment hash found %v circuits", len(openCiruits))

		// going over all circuites that needs updated
		for _, c := range openCiruits {
